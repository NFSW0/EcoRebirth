...
func _load_data():
	if FileAccess.file_exists(DATA_PATH): # 如果文件存在
		...
		if error == OK: # JSON文本解析成功
			...
			print("数据文件解析失败:",json_handler.get_error_line(),json_handler.get_error_message()) # 输出错误信息
	else: # 如果文件不存在
		save_data() # 保存新数据文件
...
func register_data(unique_name: String, value) -> bool:
	if unique_name in data: # 如果数据名重复
		print("(重复注册数据-已忽略)Name already exists. Overwriting the value.") # 输出提示信息
...
func get_data(unique_name: String, default = null):
	if unique_name in data: # 如果数据记录存在
		...
		print("(缺失数据-已忽略)Data not found:",unique_name)
